...
public class BoarLocateState : BaseState
{
    public BoarLocateState(BoarController _enemy, StateMachine _stateMachine) : base(_enemy, _stateMachine)
    {
    }
    ...
    public void FindTarget()
    {
        if (boar.target == null)
        {
            ...
            else if (boar.fenceSet.Items.Count == 0)
            {
                FindClosestSheep();
            }
        }
    ...
    private void FindClosestFence()
    {
        ...
        foreach (GameObject item in boar.fenceSet.Items)
        {
            float calcfloat = Distance(boar.transform.position, item.transform.position);
            tempL.Add(calcfloat);
            tempR.Add(item);
        }
    ...
    {
        if (boar.sheepSet.Items.Count > 0)
        {
            List<float> tempL = new List<float>();
            List<GameObject> tempR = new List<GameObject>();

            foreach (GameObject item in boar.sheepSet.Items)
            {
                float calcfloat = Distance(boar.transform.position, item.transform.position);
                tempL.Add(calcfloat);
                tempR.Add(item);
